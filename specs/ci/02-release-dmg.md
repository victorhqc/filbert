...
- **And** re-running the workflow on the same release overwrites the
  existing assets (`gh release upload --clobber`), preserves the
  maintainer-authored release description, and refreshes only the generated
  release-note section, so re-publishing a release does not produce duplicate
  or stale content
- **And** the generated release-note section is appended after the existing
  description, separated by a blank line and stable boundary markers
- **And** when the existing description is empty, the generated section is
  written without leading blank lines
...
5. **GitHub Actions release workflow.** New file
   ...
   - **Steps.** Checkout; install `create-dmg` via Homebrew; derive
     ...
     read the existing release body; remove only a previously generated
     section identified by stable boundary markers; append the current
     `<dmg>.release-notes.md` inside those markers; and update the composed
     body via `gh release edit --notes-file`. No `gh release create` — the
     trigger guarantees the release exists.
...
6. **README and release body.**
   ...
   - The **generated release note** is written by the script to
     `<dmg>.release-notes.md` and appended to the maintainer-authored GitHub
     Release description by the workflow (Plan §5). Stable boundary markers
     let a rerun refresh only this generated section. The note explains the
     unsigned-build state, the first-launch workaround, and that signed builds
     are planned. The note text is checked into the script, not generated by
     an LLM at release time.
...
- **Generated-section marker collision.** A maintainer could manually use the
  same HTML boundary markers in a release description. The markers must be
  specific to Filbert's generated release note, and the workflow must modify
  only one well-formed bounded section so unrelated prose remains untouched.
