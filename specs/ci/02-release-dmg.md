...
- **And** the DMG is uploaded to the GitHub Release as an asset, with a
  ...
  users must right-click → Open (or `xattr -dr com.apple.quarantine`) on
  first launch
...
6. **GitHub Actions release workflow.** New file
   ...
   declarative.
7. **Release body.** The workflow writes the release body. **Signed
   lane:** one-line note that the DMG is signed + notarized, plus the
   SHA-256 checksum. **Unsigned lane:** a clear first-launch workaround
   block (right-click → Open, or
   `xattr -dr com.apple.quarantine 'AI Usage.app'`) so users aren't
   surprised by Gatekeeper. The note text is checked into the script,
   not generated by an LLM at release time.
