...
final class BudgetPaceTests: XCTestCase {
    ...
    func testInitializesWithDynamicWarningBoundary() throws {
        ...
        XCTAssertEqual(
            try XCTUnwrap(allowancePercentage(of: pace, unit: .day)),
            70.0 / 6.0,
            accuracy: 0.000_1
        )
        XCTAssertEqual(
    ...
    func testEarlyWindowUsageWithinDailyAllowanceIsGood() throws {
        ...
        XCTAssertEqual(try XCTUnwrap(allowancePercentage(of: pace, unit: .day)), 13.94, accuracy: 0.01)
    ...
    private func pace(usedPercentage: Double, remainingDays: Double) -> BudgetPace? {
        BudgetPace(
            line: weeklyLine(
